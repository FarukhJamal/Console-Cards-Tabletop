...
| PR-035 | Trap Floor minimum manually playable Game Template and readable current content | Game-specific content + shared physical capabilities | G1, then Trap Floor polish | Preserve the `2-4` Player and `6 x 6` foundations; align setup/content with `2d6` starting position, orthogonal movement/shared tiles, Search as Floor Card reveal, Keys/Traps/other effects, Key-to-Exit objective, and separate permanent-hole Collapse. Old 50-coin/Floormaster/14-14-8/fixed-round systems are legacy, while unconfirmed latest-Milanote counts and listed rule values remain provisional; acceptance uses Trap Floor requirements §12 and LDR-028-LDR-030/LDR-034-LDR-039 |
