...
**Authoritative sources:** User-approved Console Cards System & Games direction supplied 2026-09-21; `Consolecards_LayoutRef_doc.pdf` and historical Russell/Milanote material remain supporting references only where not superseded; current Trap Floor authority is recorded in `18_Trap_Floor_Game_Requirements.md`; ADR-026 retains the approved shared visual/physical framework
...
| LDR-035 | Each Player uses the universal Console with Avatar in the Main Slot and eight stack-capable Side Slots for Ability/Action Cards. Controller Mapping is optional. | Game-specific setup using Platform Console | **Partial/legacy mismatch:** authored Definition data now records the approved 1+8 Slot setup, while the active prototype projection still uses its legacy six-Slot Console. | Later presentation migration without Game-rule enforcement | Trap Floor requirements §§8-11 | Avatar details and two-/three-Player layouts remain open. |
| LDR-036 | The objective is to find the selected Mode's required Keys and bring them to the Exit: Easy requires 1; Hard and Impossible require 3. | Game-specific: Trap Floor | **Partial:** current Key/Exit Runtime assistance exists, but active Mode selection is not yet driven by the authored Mode definitions. | Later Mode/setup migration without deeper rules enforcement | Trap Floor requirements §§6, 9-13 | Exact Key/Exit presentation content remains OD-018. |
